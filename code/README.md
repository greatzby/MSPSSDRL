Yes — you can copy everything below **as-is** into a local `README.md`.  
Also, below is a fully **English** version suitable for an ICML artifact/code submission.

---

# MSPSSDRL (Local README)

This file is written to be used **locally after unpacking the code** (e.g., from a ZIP).  
All commands are intended to be run from the **project root directory** unless stated otherwise.

---

## 0. Directory layout (after unpacking)
...
### 6.5.5 How decoding + labeling works (high level)
...
### 6.5.8 Practical tips / common pitfalls

- **Determinism:** if you want reproducible checkpoint-to-checkpoint comparisons, use:
  - `--temperature 0.0` and `--top-k 0` (greedy decoding)
- **Stop token mismatch:** if your preprocessing used a different termination token, the analyzer will mis-detect termination. The script assumes:
  - stop token id = `meta["stoi"]["\n"]`
- **GraphML node types:** ensure GraphML node ids are convertible via `int(node)`.
- **Step ranges:** make sure the `--step-start/end/interval` values correspond to actual saved checkpoints, or set `--ckpt-pattern` to match your filenames.
- **K must be 3 for these task types:** the `s1s3/s1s2/s2s3` decomposition is defined for K=3. For other K, you typically need a different task taxonomy and event-chain definition.

---

If you want, paste the `-h` output of your analyzer script (or the top portion that defines task types / behavior labels / event definitions), and I can tailor this section to match your **exact** column names and label definitions with zero ambiguity.
...
For additional reward shaping / masking / rollout controls (if enabled in your local code), run:
